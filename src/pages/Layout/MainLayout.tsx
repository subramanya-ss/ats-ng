...
export const MainLayout: React.FC = () => {
  ...
  const actionItems = isVacancy || isApplication
    ? [
        { label: 'Edit Candidate',             icon: <EditIcon className={classes.navIcon} /> },
        { label: 'Superuser Override',          icon: <BookmarkIcon className={classes.navIcon} /> },
        { label: 'Delete Application',          icon: <DeleteIcon className={classes.navIcon} /> },
        { label: 'Email Candidate CV',          icon: <EmailIcon className={classes.navIcon} /> },
        { label: 'Add Candidate for Vacancy',   icon: <AddIcon className={classes.navIcon} /> },
        { label: 'Add Journal Entry',           icon: <AddIcon className={classes.navIcon} /> },
      ]
    : [
  ...
  const navItems = isVacancy
    ? [
        { label: 'Home Dashboard', path: '/dashboard',  icon: DashboardIcon, active: false },
        { label: 'View Vacancy',   path: '/vacancy/1',  icon: WorkIcon,      active: true  },
      ]
    : isApplication
    ? [
        { label: 'Home Dashboard', path: '/dashboard', icon: DashboardIcon, active: false },
        { label: 'Filter Vacancy', path: '/dashboard', icon: WorkIcon,      active: false },
      ]
    : [
        { label: 'HomeDashboard',   path: '/dashboard', icon: DashboardIcon, active: isActive('/dashboard') },
        { label: 'List Dashboard',  path: '/dashboard', icon: ListAltIcon,   active: false },
      ];
  ...
  const sidebar = (
    <Box className={classes.drawerContent}>
      <Box component="nav" className={classes.nav}>
        ...
        >
          <Box className={classes.sectionBtnLeft}>
            <BoltIcon className={classes.sectionIcon} />
          ...
          <Box className={classes.subItems}>
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon className={item.active ? classes.navIconActive : classes.navIcon} />
                  <Typography className={item.active ? classes.navTextActive : classes.navText}>
                    {item.label}
                  </Typography>
                  {item.active && <Box className={classes.navActiveDot} aria-hidden="true" />}
                </Box>
              );
            })}
          </Box>
      ...
      <Box className={classes.userSection}>
        <Stack direction="row" alignItems="center" gap="12px" className={classes.userRow}>
          ...
          <Box>
            ...
            <Typography className={classes.userEmail}>admin@ats-ng.eu</Typography>
          </Box>
        </Stack>
      </Box>
    </Box>
  ...
  return (
    <Box className={classes.root}>
      ...
      <Box className={classes.body}>
        <Drawer variant="permanent" className={classes.drawer} aria-label="Main navigation">
          {sidebar}
        </Drawer>
